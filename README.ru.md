...
`k8s-lb-controller` — компактный и целенаправленный проект контроллера, построенный вокруг встроенных Kubernetes-ресурсов.
...
Этот репозиторий сделан как намеренно ограниченный по объёму проект Kubernetes-контроллера: достаточно компактный, чтобы его можно было быстро прочитать, и при этом достаточно серьёзный, чтобы показать reconcile design, finalizers, status management, backend discovery, provider integration, testing и deployability.
Он задуман как сильный публичный проект для портфолио и как лёгкий контроллерный проект для обучения, demo и контролируемых окружений, а не как полная замена системам уровня MetalLB или cloud-provider load balancers.
...
- Есть локальный сценарий разработки и сценарий деплоя через Kustomize.
...
- Helm packaging в этом репозитории пока не реализован
...
- Установка сейчас основана на Kustomize; Helm packaging планируется отдельно и не входит в текущее состояние репозитория.
...
Текущий сценарий установки основан на Kustomize.
На практике использование выглядит так:

1. Задеплоить контроллер через манифесты из `config/default` или через соответствующие `make`-таргеты.
...
Минимальный сценарий запуска внутри кластера с текущим состоянием репозитория выглядит так:
...
Минимальный пример управляемого `Service`:
...
Инструкции для Helm стоит добавлять отдельно, когда chart действительно будет реализован и опубликован.
...
С архитектурной точки зрения проект разделяет Kubernetes control-plane responsibilities и детали реализации провайдера:
...
- `internal/backends` преобразует данные `EndpointSlice` в backend endpoints для слоя провайдера.
...
Helm installation instructions здесь не приводятся, потому что Helm packaging пока не входит в текущее состояние репозитория.
...
Тестовая часть — одна из самых сильных сторон текущего репозитория.
